...
@php $header = 'Editar Transacao'; $backUrl = route('transactions.show', $transaction); @endphp

@section('content')
<form method="POST" action="{{ route('transactions.update', $transaction) }}" class="px-4 py-4 space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="text" inputmode="numeric" data-currency="amount-hidden"
                   ...
                   placeholder="0,00">
            <input type="hidden" name="amount" id="amount-hidden" value="{{ old('amount', $transaction->amount) }}">
        </div>
    ...
    <div>
        ...
        <input type="text" name="description" value="{{ old('description', $transaction->description) }}" required maxlength="255"
    ...
    <div>
        ...
        <textarea name="notes" rows="2" maxlength="1000"
                  class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('notes', $transaction->notes) }}</textarea>
    </div>
