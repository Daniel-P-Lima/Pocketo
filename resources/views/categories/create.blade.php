...
@php $header = 'Nova Categoria'; $backUrl = route('categories.index'); @endphp

@section('content')
<form method="POST" action="{{ route('categories.store') }}" class="px-4 py-4 space-y-5">
    @csrf
            ...
            @foreach(['expense' => 'Despesa', 'income' => 'Receita', 'both' => 'Ambos'] as $value => $label)
                <label class="flex-1">
                    <input type="radio" name="type" value="{{ $value }}" class="peer hidden"
                           {{ old('type', 'expense') === $value ? 'checked' : '' }}>
